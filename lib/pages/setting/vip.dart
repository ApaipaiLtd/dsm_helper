...
class _VipState extends State<Vip> {
  ...
  login(String code) async {
    var res = await Util.post("http://dsm.apaipai.top/login/app", data: {"code": code});
    if (res['code'] == 1) {
                            ...
                            Expanded(
                              child: NeuButton(
                                onPressed: () async {
                                  ...
                                  var res = await Util.post("http://dsm.apaipai.top/vip/exchange", data: {"token": userToken, "code": code.trim()});
                                  if (res['code'] == 0) {
                                    Util.toast(res['msg']);
                                  } else {
  ...
  closeMonth() async {
    if (!isLogin) {
      ...
      var res = await Util.post("http://dsm.apaipai.top/payment/wechat", data: {"token": userToken, "type": "month"});
    }
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          if (vipExpireTime != null && vipExpireTime.difference(DateTime.now()).inDays > 7)
            Text(
