...
class ShareViewController: UIViewController {
        ...
        var hostAppBundleIdentifier = "fit.flutter.dsm-helper"
    ...
    private func handleImages (content: NSExtensionItem, attachment: NSItemProvider, index: Int) {
        ...
        attachment.loadItem(forTypeIdentifier: imageContentType, options: nil) { [weak self] data, error in
             if error == nil, let url = data as? URL, let this = self {
                 ...
                 if(copied) {
                     this.sharedMedia.append(SharingFile(path: newPath.absoluteString, thumbnail: nil, duration: nil, type: .image))
                 }
    ...
    private func handleFiles (content: NSExtensionItem, attachment: NSItemProvider, index: Int) {
          attachment.loadItem(forTypeIdentifier: fileURLType, options: nil) { [weak self] data, error in
              ...
              if error == nil, let url = data as? URL, let this = self {
                  ...
                  let copied = this.copyFile(at: url, to: newPath)
                  if (copied) {
                      this.sharedMedia.append(SharingFile(path: newPath.absoluteString, thumbnail: nil, duration: nil, type: .file))
                  }
    ...
    private func getSharedMediaFile(forVideo: URL)  -> SharingFile? {
            ...
            if FileManager.default.fileExists(atPath: thumbnailPath.path) {
                return SharingFile(path: forVideo.absoluteString, thumbnail: thumbnailPath.absoluteString, duration: duration, type: .video)
            }
            ...
            return saved ? SharingFile(path: forVideo.absoluteString, thumbnail: thumbnailPath.absoluteString, duration: duration, type: .video) : nil
